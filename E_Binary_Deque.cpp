...
void solve() {
    ...
    if(sum < s) cout<<-1<<endl;
        ...
        int sub = sum-s, l=0, r=pos.size()-1;
        bool f1=true, f2=false;
        while(l<r && sub!=0) {
            if(pos[l]<(n-1-pos[r])) {l++; f1=true; f2=false;}
            else if((n-1-pos[r])<pos[l]) {r--; f2=true; f1=false;}
            else {
                if(f1) {l++; f1=true; f2=false;}
                if(f2) {r--; f2=true; f1=false;}
            }
            sub--;
        }

        if(r!=pos.size()-1 && l!=0) cout<<pos[l-1]+1+(n-1-pos[r+1])+1<<endl;
        else if(r!=pos.size()-1) cout<<(n-1-pos[r+1])+1<<endl;
        else if(l!=0) cout<<pos[l-1]+1<<endl;
    }
